...
namespace App.Server.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //[RequireHttps]
        //[AllowCrossSiteJson]
        public  ActionResult LoginAjax(LoginModel model)
        {
            ...
            try
            {
                var result = new { Errors = new Dictionary<string, List<string>>() };

                ValidateLogin(model, result.Errors);
                ...
                if (result.Errors.Count == 0)
                {
                    SetPersistentCookie(model.Username, model.RememberMe);
                    TemplateProvider tp = new TemplateProvider();
                    var okResult = new { UserName = model.Username, Token =  tp.GetTemplate("AntiForgeryToken", null, this, true) };
                    json.Data = okResult;
        ...
        private void ValidateLogin(LoginModel login, Dictionary<string, List<string>> errors)
        {
            if (CxUtils.IsEmpty(login.Username))
                Utils.AddError(errors, "UserName", "User name could not be empty.");
            if (CxUtils.IsEmpty(login.Password))
                Utils.AddError(errors, "Password", "Password could not be empty.");
            CxMetadataHolder metadata = 
                (CxSlMetadataHolder)System.Web.HttpContext.Current.Application[CxAppServerConsts.METADATA_APP_KEY];
            if (metadata == null)
            ...
            using (CxDbConnection connection = CxDbConnections.CreateEntityConnection())
            {
                try
                {
                    CxUserInfo userInfo = metadata.Security.ValidateLogin(connection, login.Username, login.Password, true);
                }
                catch(ExValidationException ex)
                {
                    if(ex.Message.Contains("User name") || ex.Message.Contains("User"))
                        Utils.AddError(errors, "UserName", ex.Message);
                    if (ex.Message.Contains("password"))
                        Utils.AddError(errors, "Password", ex.Message);
                }
            }
