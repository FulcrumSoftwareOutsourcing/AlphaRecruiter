...
namespace Framework.Remote
{
  public partial class CxAppServer
  {
    ...
    public CxExpressionResult CalculateExpressions(CxQueryParams prms)
    {
      try
      {
        ...
        foreach (CxAttributeMetadata attribute in meta.Attributes)
        {
          ...
          entityValues.Add(clientAttributeMetadata, prms.EntityValues[attribute.Id]);
        }
        ...
        foreach (CxAttributeMetadata attribute in meta.Attributes)
        {
          ...
          if (attribute.PrimaryKey ||
              (prms.EntityValues.ContainsKey(attribute.Id) &&
              !CxUtils.Compare(val, prms.EntityValues[attribute.Id])))
          {
      ...
      {
        CxExpressionResult withError = new CxExpressionResult();
        CxExceptionDetails exceptionDetails = new CxExceptionDetails(ex);
        withError.Error = exceptionDetails;
        return withError;
      }
    }
